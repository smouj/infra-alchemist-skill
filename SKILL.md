...
description: Transmute complex infrastructure configurations into pure operational elegance through automated, self-healing workflows
author: Kilo-Core
version: 1.0.0
tags:
  - cloud
...
Infrastructure Alchemist turns declarative infrastructure definitions into reliable, production-ready deployments with built-in safety nets and self-healing capabilities.

**Real use cases:**
- Deploy multi-region AWS/GCP/Azure infrastructure with terraform and automated failover
- Implement GitOps workflows using ArgoCD/Flux with automated drift detection and reconciliation
- Create self-healing Kubernetes clusters that automatically restart failed pods, replace unhealthy nodes, and scale based on custom metrics
- Build automated disaster recovery pipelines that snapshot databases, backup etcd, and replicate configurations across regions
- Optimize cloud costs through automated right-sizing of resources based on usage patterns
- Migrate legacy infrastructure to modern IaC with state import and validation
- Manage complex secrets injection using HashiCorp Vault or cloud KMS
- Set up multi-account AWS organizations with SCPs, cross-account roles, and centralized logging
...
1. **Intent Parsing:** Analyze user prompt to identify target platform (AWS/GCP/Azure/K8s), operation type (deploy/update/destroy/heal), and scope (single resource vs. entire stack)
2. **Environment Detection:** Auto-detect context from KUBECONFIG, terraform workspace, AWS_PROFILE, GOOGLE_APPLICATION_CREDENTIALS; fail fast if missing
3. **Dependency Resolution:** Check required binaries (terraform version >= 1.0, kubectl cluster connectivity, cloud CLI auth); install missing via package manager if configured
4. **Pre-Flight Validation:**
   - Run `terraform validate` for IaC changes
   - Verify `kubectl cluster-info` for K8s operations
   - Check `aws sts get-caller-identity` for cloud auth
   - Ensure state bucket/backend accessible
   - Confirm sufficient cloud quotas
5. **Plan Generation:** Create detailed execution plan with `terraform plan -out=tfplan` or `kubectl apply --dry-run=client -o yaml`; display human-readable summary with count of resources to change
6. **Safety Gates:**
   - Detect destroy operations; require explicit approval flag `--approve-destroy` if > 5 resources
   - Check for potential data loss (RDS deletion protection disabled, EBS volumes without snapshot)
   - Verify replica count won't drop below minimum
   - Ensure maintenance windows for production environments
7. **Execute with Logging:** Run commands with real-time output streaming; capture full stdout/stderr to `./logs/infra-alchemist-$(date +%Y%m%d-%H%M%S).log`
8. **Post-Execution Health Checks:**
   - Wait for K8s pods to reach Ready state (`kubectl wait --for=condition=Ready pod --all --timeout=180s`)
   - Verify load balancer health checks passing
   - Run integration test endpoint checks if available
   - Confirm terraform outputs match expected values
9. **State Management:** Backup terraform state to S3/GCS/Azure Blob; commit plan files to git for audit trail; prune old backups > 30 days
10. **Summary Generation:** Produce human-readable report with resource IDs, endpoint URLs, credentials locations, rollback instructions, and next maintenance窗口

## Golden Rules

1. **Never skip terraform plan** - Always review changes before apply; plan must be stored in git
2. **State backups mandatory** - Backup terraform state before every apply; keep 3 latest versions
3. **No secrets in plaintext** - Never pass passwords via `-var 'password=secret'`; use vault or `-var-file` with restricted permissions
4. **Minimum blast radius** - Use terraform workspaces, K8s namespaces, and cloud tags to isolate environments; limit production changes to single service when possible
5. **Always preserve deletion protection** - RDS, Redshift, and other databases must have deletion protection enabled in production
6. **Require health checks** - Post-deployment validation required; if health check fails, automatically trigger rollback
7. **Immutability principle** - Never modify existing resources in-place; prefer replace-with-no-downtime patterns using blue/green or rolling updates
8. **Tag everything** - All cloud resources must have tags: Environment, Owner, ManagedBy=infra-alchemist, GitCommit
9. **Assume network partitions** - Design for partial failures; use retry logic with exponential backoff in custom scripts
10. **Rollback before forward** - For any production change, verify rollback procedure works in staging first

## Examples

### Example 1: Deploy Multi-Tier Web Application

**User prompt:** "Deploy production web app in AWS: VPC with public/private subnets, ECS cluster with ALB, RDS PostgreSQL, and ElastiCache Redis. Use production.tfvars."
...
### Example 2: Self-Healing Kubernetes Namespace

**User prompt:** "Heal the production namespace: restart all CrashLoopBackOff pods, cordon and drain nodes with memory pressure, and verify all deployments have min 2 replicas."
...
### Example 3: Terraform Drift Detection and Remediation

**User prompt:** "Detect and report any drift in the production terraform state. Auto-remediate if safe, otherwise flag for manual review."
...
### Cloud Provider Specific Rollbacks
...
### Emergency Global Rollback (All Services)
```bash
#!/bin/bash
...
- terraform (v1.0+)
- kubectl (v1.24+)
- awscli OR gcloud OR az (at least one cloud provider)
- jq (v1.6+)
- yq (v4.30+)
- helm (v3.10+, optional)
- git
...
**Terraform backend configuration (required):**
```hcl
terraform {
  backend "s3" {
